...
The **In-Sample Error** (IS) rates shown in **Figure 3** were calculated using the predictions from the models built in the **Data Partitioning and Model Builds** section on the **training** data.  The **Out-of-Sample Error** (OOS) rates in the same figure were calculated using the samples held out in the **testing** data set.  The IS error rates were very close to the OOS error rates.  Because we would expect IS error rates to be smaller than OOS, it was a little unusual to see IS error rates being larger than OOS error rates in 2 of the 4 cases.  I would attribute the LDA and RPART anomalies to resampling error since the differences were very small.
